...
(0, _src_1.describeForkTest)('SNXRecoveryCoordinator', 'mainnet', 14945041, function () {
    let govMultisig;
    let coordinator;
    let vault;
    let authorizer;
    let allowlistTokenRole, withdrawCollectedFeesRole;
    let task;
    ...
    before('run task', async () => {
        task = new _src_2.Task('20220610-snx-recovery-coordinator', _src_2.TaskMode.TEST, (0, _src_3.getForkedNetwork)(hardhat_1.default));
    ...
    before('grant permissions', async () => {
        ...
        const protocolFeeWithdrawerTask = new _src_2.Task('20220517-protocol-fee-withdrawer', _src_2.TaskMode.READ_ONLY, (0, _src_3.getForkedNetwork)(hardhat_1.default));
        ...
        allowlistTokenRole = await (0, actions_1.actionId)(protocolFeeWithdrawer, 'allowlistToken');
        withdrawCollectedFeesRole = await (0, actions_1.actionId)(protocolFeeWithdrawer, 'withdrawCollectedFees');
        await authorizer
            .connect(govMultisig)
    ...
    it('reverts on large exits from the sBTC pool', async () => {
        ...
        const whale = await (0, _src_4.impersonate)(BTC_STABLE_POOL_WHALE);
        await (0, chai_1.expect)(vault.connect(whale).exitPool(BTC_STABLE_POOL_ID, BTC_STABLE_POOL_WHALE, BTC_STABLE_POOL_WHALE, {
            ...
            userData: abi_1.defaultAbiCoder.encode(['uint256', 'uint256'], [EXACT_BPT_IN_FOR_TOKENS_OUT, await poolContract.balanceOf(BTC_STABLE_POOL_WHALE)]),
    ...
    it('reverts on large exits from the SNX pool', async () => {
        ...
        const whale = await (0, _src_4.impersonate)(SNX_WEIGHTED_POOL_WHALE);
        await (0, chai_1.expect)(vault.connect(whale).exitPool(SNX_WEIGHTED_POOL_ID, SNX_WEIGHTED_POOL_WHALE, SNX_WEIGHTED_POOL_WHALE, {
            assets: [SNX, WETH],
            minAmountsOut: [0, 0],
            userData: encoder_1.WeightedPoolEncoder.exitExactBPTInForTokensOut(await poolContract.balanceOf(SNX_WEIGHTED_POOL_WHALE)),
            toInternalBalance: false,
    ...
    it('transfers tokens to the vault', async () => {
        ...
        (0, expectTransfer_1.expectTransferEvent)(await tx.wait(), { from: await vault.getProtocolFeesCollector(), to: vault.address, value: SNX_AMOUNT }, SNX);
        (0, expectTransfer_1.expectTransferEvent)(await tx.wait(), { from: await vault.getProtocolFeesCollector(), to: vault.address, value: sBTC_AMOUNT }, sBTC);
    });
    it('renounces its permissions to allowlist tokens on the ProtocolFeesWithdrawer', async () => {
        (0, chai_1.expect)(await authorizer.hasRole(allowlistTokenRole, coordinator.address)).to.be.false;
    });
    it('renounces its permissions to withdraw tokens through the ProtocolFeesWithdrawer', async () => {
        (0, chai_1.expect)(await authorizer.hasRole(withdrawCollectedFeesRole, coordinator.address)).to.be.false;
    });
