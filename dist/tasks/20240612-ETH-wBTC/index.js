...
exports.default = async (task, { force, from } = {}) => {
    ...
    if (task.mode === _src_2.TaskMode.LIVE) {
        ...
        const newWeightedPoolParams = {
            name: input.NAME,
            symbol: input.SYMBOL,
            tokens: [input.WBTC, input.WETHER].sort(function (a, b) {
        ...
        if (force || !task.output({ ensure: false })['ETHBTCWeightedPool']) {
            ...
            await task.save({ ETHBTCWeightedPool: mockPoolAddress });
        }
        const realPool = await task.instanceAt('WeightedPool', task.output()['ETHBTCWeightedPool']);
        // Recupera la poolId dal contratto della pool
        const poolId = await realPool.getPoolId();
        console.log(`Pool ID: ${poolId}`);
        // Salva la poolId nel task output
        await task.save({ ETHBTCWeightedPoolId: poolId });
