...
exports.default = async (task, { force, from } = {}) => {
    ...
    if (task.mode === _src_2.TaskMode.LIVE) {
        ...
        const newWeightedPoolParams = {
            name: input.NAME,
            symbol: input.SYMBOL,
            tokens: [input.WETH, input.WETHER].sort(function (a, b) {
                return a.toLowerCase().localeCompare(b.toLowerCase());
            }),
            normalizedWeights: [(0, numbers_1.fp)(0.5), (0, numbers_1.fp)(0.5)],
        ...
        if (force || !task.output({ ensure: false })['IOTAETHWeightedPool']) {
            ...
            await task.save({ IOTAETHWeightedPool: mockPoolAddress });
        }
        const realPool = await task.instanceAt('WeightedPool', task.output()['IOTAETHWeightedPool']);
        // Recupera la poolId dal contratto della pool
        const poolId = await realPool.getPoolId();
        console.log(`Pool ID: ${poolId}`);
        // Salva la poolId nel task output
        await task.save({ IOTAETHWeightedPoolId: poolId });
