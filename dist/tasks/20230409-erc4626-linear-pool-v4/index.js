...
exports.default = async (task, { force, from } = {}) => {
    ...
    if (task.mode === _src_2.TaskMode.LIVE) {
        ...
        if (force || !task.output({ ensure: false })['MockERC4626LinearPool']) {
            ...
            const poolCreationReceipt = await (await factory.create(mockPoolArgs.name, mockPoolArgs.symbol, mockPoolArgs.mainToken, mockPoolArgs.wrappedToken, mockPoolArgs.upperTarget, mockPoolArgs.swapFeePercentage, mockPoolArgs.owner, PROTOCOL_ID, SALT)).wait();
        ...
        const vaultTask = new _src_2.Task('20210418-vault', _src_2.TaskMode.READ_ONLY, task.network);
