...
exports.default = async (task, { force, from } = {}) => {
    ...
    if (task.mode === _src_1.TaskMode.LIVE) {
        ...
        const mockPoolArgs = {
            ...
            name: 'USDT/USDC Composable Stable Pool',
            symbol: 'USDTUSDC',
            tokens: [input.USDT, input.USDC].sort(function (a, b) {
        ...
        if (force || !task.output({ ensure: false })['USDTUSDCComposableStablePool']) {
            const poolCreationReceipt = await (await factory.create(mockPoolArgs.name, mockPoolArgs.symbol, mockPoolArgs.tokens, mockPoolArgs.amplificationParameter, mockPoolArgs.rateProviders, mockPoolArgs.tokenRateCacheDurations, mockPoolArgs.exemptFromYieldProtocolFeeFlag, mockPoolArgs.swapFeePercentage, mockPoolArgs.owner, constants_1.ZERO_BYTES32)).wait();
            ...
            await task.save({ USDTUSDCComposableStablePool: mockPoolAddress });
        }
        const mockPool = await task.instanceAt('ComposableStablePool', task.output()['USDTUSDCComposableStablePool']);
        // Recupera la poolId dal contratto della pool
        const poolId = await mockPool.getPoolId();
        console.log(`Pool ID: ${poolId}`);
        // Salva la poolId nel task output
        await task.save({ ComposableStablePoolId: poolId });
