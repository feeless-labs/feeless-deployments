...
(0, _src_1.describeForkTest)('NoProtocolFeeLiquidityBootstrappingPoolFactory', 'mainnet', 14850000, function () {
    let owner, whale;
    ...
    before('run task', async () => {
        task = new _src_1.Task('20211202-no-protocol-fee-lbp', _src_1.TaskMode.TEST, (0, _src_1.getForkedNetwork)(hardhat_1.default));
    ...
    it('authorized accounts can disable the factory', async () => {
        ...
        const admin = await (0, _src_1.impersonate)(await authorizer.getRoleMember(DEFAULT_ADMIN_ROLE, 0));
        await authorizer.connect(admin).grantRole(await (0, actions_1.actionId)(factory, 'disable'), admin.address);
        await factory.connect(admin).disable();
        (0, chai_1.expect)(await factory.isDisabled()).to.equal(true);
    });
    it('disabling prevents pool creation', async () => {
        await (0, chai_1.expect)(factory.create('LBP', 'LBPT', tokens, initialWeights, swapFeePercentage, owner.address, swapEnabledOnStart)).to.be.revertedWith('BAL#211'); // Errors.DISABLED
    });
});
