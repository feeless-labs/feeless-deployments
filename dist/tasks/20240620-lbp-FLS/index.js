...
exports.default = async (task, { force, from } = {}) => {
    ...
    if (task.mode === _src_1.TaskMode.LIVE) {
        ...
        const newPoolParams = {
            ...
            weights: [(0, numbers_1.fp)(0.2), (0, numbers_1.fp)(0.8)],
            swapFeePercentage: (0, numbers_1.bn)(1e12),
            swapEnabledOnStart: true,
            owner: constants_1.ZERO_ADDRESS,
        };
        // This mimics the logic inside task.deploy
        if (force || !task.output({ ensure: false })['FLSLiquidityBootstrappingPool']) {
            ...
            await task.save({ FLSLiquidityBootstrappingPool: mockPoolAddress });
        }
        const mockPool = await task.instanceAt('LiquidityBootstrappingPool', task.output()['FLSLiquidityBootstrappingPool']);
        // Recupera la poolId dal contratto della pool
        const poolId = await mockPool.getPoolId();
        console.log(`Pool ID: ${poolId}`);
        // Salva la poolId nel task output
        await task.save({ FLSLiquidityBootstrappingPoolId: poolId });
