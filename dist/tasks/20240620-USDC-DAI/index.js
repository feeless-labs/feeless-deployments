...
exports.default = async (task, { force, from } = {}) => {
    ...
    if (task.mode === _src_1.TaskMode.LIVE) {
        ...
        const mockPoolArgs = {
            ...
            name: 'USDC/DAI Composable Stable Pool',
            symbol: 'USDCDAI',
            tokens: [input.USDC, input.DAI].sort(function (a, b) {
        ...
        if (force || !task.output({ ensure: false })['USDCDAIComposableStablePool']) {
            ...
            await task.save({ USDCDAIComposableStablePool: mockPoolAddress });
        }
        const mockPool = await task.instanceAt('ComposableStablePool', task.output()['USDCDAIComposableStablePool']);
