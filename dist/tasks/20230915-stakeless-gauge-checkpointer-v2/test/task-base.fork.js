...
(0, _src_1.describeForkTest)('StakelessGaugeCheckpointer V2 - Base', 'mainnet', 17332499, function () {
    ...
    before('run task', async () => {
        task = new _src_2.Task('20230915-stakeless-gauge-checkpointer-v2', _src_2.TaskMode.TEST, (0, _src_3.getForkedNetwork)(hardhat_1.default));
    ...
    before('add gauges to checkpointer', async () => {
        ...
        await Promise.all(Array.from(gauges).map(([gaugeType, gaugesData]) => {
            stakelessGaugeCheckpointer.connect(admin).addGaugesWithVerifiedType(GaugeType[gaugeType], gaugesData.map((gaugeData) => gaugeData.address));
        }));
    });
    ...
    it('checks that gauges were added correctly', async () => {
        for (const [gaugeType, gaugesData] of gauges.entries()) {
            (0, chai_1.expect)(await stakelessGaugeCheckpointer.getTotalGauges(GaugeType[gaugeType])).to.be.eq(gaugesData.length);
        }
    });
    describe('getTotalBridgeCost', () => {
        function itChecksTotalBridgeCost(minRelativeWeight) {
            it('checks total bridge cost', async () => {
                ...
                (0, chai_1.expect)(await stakelessGaugeCheckpointer.getTotalBridgeCost(minRelativeWeight)).to.be.eq(singleGaugeBridgeCost.mul(gaugesAmountAboveMinWeight));
            });
        }
    ...
    describe('getSingleBridgeCost', () => {
        it('gets the cost for an arbitrum gauge', async () => {
            ...
            (0, chai_1.expect)(await stakelessGaugeCheckpointer.getSingleBridgeCost(arbitrumType, arbitrumGauge.address)).to.be.eq(bridgeCost);
        });
        it('gets the cost for an non-arbitrum gauge', async () => {
            ...
            (0, chai_1.expect)(await stakelessGaugeCheckpointer.getSingleBridgeCost(gnosisType, gnosisGauge.address)).to.be.eq(0);
        });
        it('reverts when the gauge address is not present in the type', async () => {
            const gnosisGauge = await task.instanceAt('GnosisRootGauge', gauges.get(GaugeType.Gnosis)[0].address);
            const polygonType = GaugeType[GaugeType.Polygon];
            await (0, chai_1.expect)(stakelessGaugeCheckpointer.getSingleBridgeCost(polygonType, gnosisGauge.address)).to.be.revertedWith('Gauge not added');
        });
    });
    describe('checkpoint', () => {
        ...
        function itCheckpointsGaugesAboveRelativeWeight(minRelativeWeight, gaugesAboveThreshold) {
            ...
            context('when checkpointing only Ethereum gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypesAboveRelativeWeight([GaugeType[GaugeType.Ethereum]], minRelativeWeight);
                        return await tx.wait();
                    };
            ...
            context('when checkpointing only Polygon gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypesAboveRelativeWeight([GaugeType[GaugeType.Polygon]], minRelativeWeight);
                        return await tx.wait();
                    };
            ...
            context('when checkpointing only Arbitrum gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypesAboveRelativeWeight([GaugeType[GaugeType.Arbitrum]], minRelativeWeight, {
                            value: await stakelessGaugeCheckpointer.getTotalBridgeCost(minRelativeWeight),
                        });
                        return await tx.wait();
                    };
            ...
            context('when checkpointing only Optimism gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypesAboveRelativeWeight([GaugeType[GaugeType.Optimism]], minRelativeWeight);
                        return await tx.wait();
                    };
            ...
            context('when checkpointing only Gnosis gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypesAboveRelativeWeight([GaugeType[GaugeType.Gnosis]], minRelativeWeight);
                        return await tx.wait();
                    };
            ...
            context('when checkpointing multiple gauges (single type)', () => {
                sharedBeforeEach(async () => {
                    gaugeDataAboveMinWeight = gnosisGaugeDataAboveMinWeight;
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointMultipleGaugesOfMatchingType(GaugeType[GaugeType.Gnosis], gaugeDataAboveMinWeight.map((gaugeData) => gaugeData.address));
                        return await tx.wait();
                    };
                });
                itPerformsCheckpoint();
            ...
            context('when checkpointing multiple gauges (multiple types)', () => {
                sharedBeforeEach(async () => {
                    ...
                    const value = await stakelessGaugeCheckpointer.getGaugeTypesBridgeCost([GaugeType[GaugeType.Arbitrum], GaugeType[GaugeType.Polygon]], minRelativeWeight);
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointMultipleGauges(matchingTypesArray, gaugeDataAboveMinWeight.map((gaugeData) => gaugeData.address), { value });
                        return await tx.wait();
                    };
                });
                itPerformsCheckpoint();
