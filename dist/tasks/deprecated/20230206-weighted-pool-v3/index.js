...
exports.default = async (task, { force, from } = {}) => {
    ...
    if (task.mode === _src_2.TaskMode.LIVE) {
        ...
        if (force || !task.output({ ensure: false })['MockWeightedPool']) {
            const poolCreationReceipt = await (await factory.create(mockPoolArgs.params.name, mockPoolArgs.params.symbol, mockPoolArgs.params.tokens, mockPoolArgs.params.normalizedWeights, mockPoolArgs.params.rateProviders, mockPoolArgs.params.swapFeePercentage, mockPoolArgs.owner)).wait();
