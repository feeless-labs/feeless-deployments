...
(0, _src_2.describeForkTest)('YearnLinearPoolFactory', 'mainnet', 16610000, function () {
    ...
    const INITIAL_UPPER_TARGET = (0, numbers_1.fp)(1e5);
    // The initial midpoint (upper target / 2) must be between the final lower and upper targets
    const FINAL_LOWER_TARGET = (0, numbers_1.fp)(0.2e5);
    const FINAL_UPPER_TARGET = (0, numbers_1.fp)(5e5);
    const PROTOCOL_ID = 3;
    let AttackType;
    (function (AttackType) {
    ...
    before('run task', async () => {
        task = new _src_1.Task('20230213-yearn-linear-pool', _src_1.TaskMode.TEST, (0, _src_1.getForkedNetwork)(hardhat_1.default));
    ...
    describe('create and check getters', () => {
        it('deploy a linear pool', async () => {
            const tx = await factory.create('', '', USDC, yvUSDC, INITIAL_UPPER_TARGET, SWAP_FEE_PERCENTAGE, owner.address, PROTOCOL_ID);
    ...
    describe('rebalancer query protection', async () => {
        it('reverts with a malicious lending pool', async () => {
            ...
            await (0, hardhat_network_helpers_1.setCode)(yvUSDC, (0, _src_3.getArtifact)('MockYearnTokenVault').deployedBytecode);
            const mockYearnTokenVault = await (0, _src_3.instanceAt)('MockYearnTokenVault', yvUSDC);
