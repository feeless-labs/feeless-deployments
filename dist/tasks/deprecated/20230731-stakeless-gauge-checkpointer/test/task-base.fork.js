...
(0, _src_1.describeForkTest)('StakelessGaugeCheckpointer - Base', 'mainnet', 17332499, function () {
    ...
    describe('getSingleBridgeCost', () => {
        ...
        it('reverts when the gauge address is not present in the type', async () => {
            ...
            await (0, chai_1.expect)(stakelessGaugeCheckpointer.getSingleBridgeCost(polygonType, gnosisGauge.address)).to.be.revertedWith('Gauge was not added to the checkpointer');
        });
    });
    describe('checkpoint', () => {
        ...
        function itCheckpointsGaugesAboveRelativeWeight(minRelativeWeight, gaugesAboveThreshold) {
            ...
            context('when checkpointing only Ethereum gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypeAboveRelativeWeight(GaugeType[GaugeType.Ethereum], minRelativeWeight);
                        return await tx.wait();
                    };
            ...
            context('when checkpointing only Polygon gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypeAboveRelativeWeight(GaugeType[GaugeType.Polygon], minRelativeWeight);
                        return await tx.wait();
                    };
            ...
            context('when checkpointing only Arbitrum gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypeAboveRelativeWeight(GaugeType[GaugeType.Arbitrum], minRelativeWeight, {
                            value: await stakelessGaugeCheckpointer.getTotalBridgeCost(minRelativeWeight),
                        });
                        return await tx.wait();
                    };
            ...
            context('when checkpointing only Optimism gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypeAboveRelativeWeight(GaugeType[GaugeType.Optimism], minRelativeWeight);
                        return await tx.wait();
                    };
            ...
            context('when checkpointing only Gnosis gauges', () => {
                sharedBeforeEach(async () => {
                    performCheckpoint = async () => {
                        const tx = await stakelessGaugeCheckpointer.checkpointGaugesOfTypeAboveRelativeWeight(GaugeType[GaugeType.Gnosis], minRelativeWeight);
                        return await tx.wait();
                    };
