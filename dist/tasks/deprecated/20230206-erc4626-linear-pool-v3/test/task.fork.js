...
(0, _src_2.describeForkTest)('ERC4626LinearPoolFactory', 'mainnet', 16550500, function () {
    ...
    before('run task', async () => {
        task = new _src_1.Task('20230206-erc4626-linear-pool-v3', _src_1.TaskMode.TEST, (0, _src_1.getForkedNetwork)(hardhat_1.default));
    ...
    describe('create and check getters', () => {
        it('deploy a linear pool', async () => {
            const tx = await factory.create('', '', frxEth, erc4626Token, INITIAL_UPPER_TARGET, SWAP_FEE_PERCENTAGE, owner.address, PROTOCOL_ID);
    ...
    describe('read-only reentrancy protection', () => {
        ...
        before('deploy pool and prepare', async () => {
            const tx = await factory.create('', '', WETH, erc4626Token, INITIAL_UPPER_TARGET, SWAP_FEE_PERCENTAGE, owner.address, PROTOCOL_ID);
            const event = expectEvent.inReceipt(await tx.wait(), 'PoolCreated');
