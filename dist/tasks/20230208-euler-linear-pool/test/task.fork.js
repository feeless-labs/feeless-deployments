...
(0, _src_2.describeForkTest)('EulerLinearPoolFactory', 'mainnet', 16550500, function () {
    ...
    before('run task', async () => {
        task = new _src_1.Task('20230208-euler-linear-pool', _src_1.TaskMode.TEST, (0, _src_1.getForkedNetwork)(hardhat_1.default));
    ...
    describe('create and check getters', () => {
        it('deploy a linear pool', async () => {
            const tx = await factory.create('', '', USDC, eUSDC, INITIAL_UPPER_TARGET, SWAP_FEE_PERCENTAGE, owner.address, PROTOCOL_ID);
        ...
        it('check factory aware of EULER_PROTOCOL', async () => {
            (0, chai_1.expect)(await factory.eulerProtocol()).to.equal(EULER_PROTOCOL);
        });
        it('check Rebalancer aware of EULER_PROTOCOL', async () => {
            (0, chai_1.expect)(await rebalancer.eulerProtocol()).to.equal(EULER_PROTOCOL);
        });
    ...
    describe('rebalancer query protection', async () => {
        it('reverts with a malicious lending pool', async () => {
            ...
            await (0, hardhat_network_helpers_1.setCode)(eUSDC, (0, _src_3.getArtifact)('MockEulerToken').deployedBytecode);
            const mockLendingPool = await (0, _src_3.instanceAt)('MockEulerToken', eUSDC);
    ...
    describe('read-only reentrancy protection', () => {
        ...
        before('deploy attacker', async () => {
            attacker = await (0, _src_3.deploy)('ReadOnlyReentrancyAttackerEP', [vault.address]);
        });
