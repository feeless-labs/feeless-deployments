...
(0, _src_1.describeForkTest)('ComposableStablePool V6', 'mainnet', 19292000, function () {
    let task;
    let factory;
    let owner;
    let whale, auraWhale, graviAuraWhale;
    let govMultisig;
    let vault;
    let authorizer;
    let dai, usdc, aura, graviAura;
    ...
    let factoryDeploymentTimestamp;
    let AttackType;
    (function (AttackType) {
    ...
    before('run task', async () => {
        task = new _src_2.Task('20240223-composable-stable-pool-v6', _src_2.TaskMode.TEST, (0, _src_3.getForkedNetwork)(hardhat_1.default));
        await task.run({ force: true });
        factory = await task.deployedInstance('ComposableStablePoolFactory');
        factoryDeploymentTimestamp = await (0, time_1.currentTimestamp)();
    });
    before('load signers', async () => {
        ...
        graviAuraWhale = await (0, _src_4.impersonate)(GRAVI_AURA_HOLDER, (0, numbers_1.fp)(100));
    ...
    async function createPool(tokens, rateProvider = constants_1.ZERO_ADDRESS, initialize = true, salt = '') {
        ...
        const tx = await factory.create('CSP', 'CSBPT', tokens, amplificationParameter, rateProviders, cacheDurations, exemptFlag, swapFeePercentage, owner.address, salt == '' ? (0, utils_1.randomBytes)(32) : salt);
        ...
        context('proportional join', () => {
            ...
            it('joins proportionally', async () => {
                ...
                const adjustedBalances = [
                    initialBalanceDAI.div((0, numbers_1.fp)(4.99)).mul((0, numbers_1.fp)(1)),
    ...
    describe('pause window', () => {
        const EXPECTED_PAUSE_WINDOW = 4 * 365 * time_1.DAY;
        const EXPECTED_BUFFER_PERIOD = 180 * time_1.DAY;
        (0, sharedBeforeEach_1.sharedBeforeEach)(async () => {
            // Reset timestamp and double check that we're close to factory deployment time.
            (0, chai_1.expect)(await (0, time_1.currentTimestamp)()).to.be.lt(factoryDeploymentTimestamp.add(time_1.MINUTE));
        });
        it('can be paused until 4 years after factory deployment', async () => {
            const pool = await createPool(tokens, constants_1.ZERO_ADDRESS, false, constants_1.ZERO_BYTES32);
            await authorizer.connect(govMultisig).grantRole(await (0, actions_1.actionId)(pool, 'pause'), govMultisig.address);
            await (0, time_1.advanceTime)(EXPECTED_PAUSE_WINDOW - time_1.MINUTE);
        ...
        it('cannot be paused more than 4 years after factory deployment', async () => {
            ...
            await authorizer.connect(govMultisig).grantRole(await (0, actions_1.actionId)(pool, 'pause'), govMultisig.address);
            await (0, time_1.advanceTime)(EXPECTED_PAUSE_WINDOW + time_1.MINUTE);
            await (0, chai_1.expect)(pool.connect(govMultisig).pause()).to.be.revertedWith('BAL#403');
        });
        it('stays paused for 6 months after the pause window expires', async () => {
            const pool = await createPool(tokens, constants_1.ZERO_ADDRESS, false, constants_1.ZERO_BYTES32);
            await authorizer.connect(govMultisig).grantRole(await (0, actions_1.actionId)(pool, 'pause'), govMultisig.address);
            await pool.connect(govMultisig).pause();
            await (0, time_1.advanceTime)(EXPECTED_PAUSE_WINDOW + EXPECTED_BUFFER_PERIOD - time_1.MINUTE);
            const pausedState = await pool.getPausedState();
            (0, chai_1.expect)(pausedState.paused).to.be.true;
        });
    });
