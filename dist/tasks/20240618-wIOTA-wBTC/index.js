...
exports.default = async (task, { force, from } = {}) => {
    ...
    if (task.mode === _src_2.TaskMode.LIVE) {
        ...
        const newWeightedPoolParams = {
            name: input.NAME,
            symbol: input.SYMBOL,
            tokens: [input.WETH, input.WBTC].sort(function (a, b) {
                return a.toLowerCase().localeCompare(b.toLowerCase());
            }),
            normalizedWeights: [(0, numbers_1.fp)(0.4), (0, numbers_1.fp)(0.6)],
        ...
        if (force || !task.output({ ensure: false })['IOTABTCWeightedPool']) {
            ...
            await task.save({ IOTABTCWeightedPool: mockPoolAddress });
        }
        const realPool = await task.instanceAt('WeightedPool', task.output()['IOTABTCWeightedPool']);
        // Recupera la poolId dal contratto della pool
        const poolId = await realPool.getPoolId();
        console.log(`Pool ID: ${poolId}`);
        // Salva la poolId nel task output
        await task.save({ IOTABTCWeightedPoolId: poolId });
